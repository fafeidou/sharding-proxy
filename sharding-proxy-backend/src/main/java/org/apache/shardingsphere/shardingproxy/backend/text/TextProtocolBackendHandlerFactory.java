...
public final class TextProtocolBackendHandlerFactory {
     ...
     */
    public static TextProtocolBackendHandler newInstance(final DatabaseType databaseType, final String sql,
        ...
        SQLStatement sqlStatement = new SQLParseKernel(ParseRuleRegistry.getInstance(), databaseType.getName(), sql)
            .parse();
        ...
        QueryBackendHandler queryBackendHandler = new QueryBackendHandler(sql, backendConnection);
        return queryBackendHandler;
    }

    private static TextProtocolBackendHandler createTCLBackendHandler(final String sql, final TCLStatement tclStatement,
        final BackendConnection backendConnection) {
        ...
        if (tclStatement instanceof SetAutoCommitStatement) {
            if (((SetAutoCommitStatement) tclStatement).isAutoCommit()) {
                return backendConnection.getStateHandler().isInTransaction() ? new TransactionBackendHandler(
    ...
    private static TextProtocolBackendHandler createDALBackendHandler(final DALStatement dalStatement, final String sql,
        final BackendConnection backendConnection) {
